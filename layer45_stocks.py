...
def fetch_financial_data(code: str) -> dict:
    ...
    # ── 1. 东方财富个股信息 → 总市值/总股本（PE/PB 在新版 akshare 已无此列） ──
    def _fetch_info():
    ...
    if df is not None and len(df) > 0:
        try:
            ...
            if "item" in df.columns:
                ...
                _info_lookup = lambda k: df.loc[k, val_col] if k in df.index else None
            else:
                first_col = df.columns[0]
                df = df.set_index(first_col)
                _info_lookup = lambda k: df.loc[k, df.columns[0]] if k in df.index else None

            pe_val = _info_lookup("市盈率-动态")
            if pe_val is not None:
                result["pe"] = _parse_pct(pe_val)
            pb_val = _info_lookup("市净率")
            if pb_val is not None:
                result["pb"] = _parse_pct(pb_val)
            mktcap_val = _info_lookup("总市值")
            if mktcap_val is not None:
                mktcap_num = _parse_pct(mktcap_val)
                if mktcap_num and mktcap_num > 1e8:
                    result["mktcap"] = f"{mktcap_num/1e8:.0f}亿"
                else:
                    result["mktcap"] = str(mktcap_val)
        except Exception:
            pass

    # ── 2. 同花顺财务摘要（净利润/营收/毛利率/增长率/eps 等） ──
    def _fetch_abstract():
        return ak.stock_financial_abstract_ths(symbol=code, indicator="按报告期")
    df = _fetch_with_retry(_fetch_abstract)
    if df is not None and len(df) > 0:
        try:
            row = df.tail(1).iloc[0]  # 取最新报告期（df按时间升序，最后一行最新）
            result["gross_margin"] = _parse_pct(row.get("销售毛利率"))
            result["net_margin"] = _parse_pct(row.get("销售净利率"))
            result["roe"] = _parse_pct(row.get("净资产收益率"))
            result["rev_yoy"] = _parse_pct(row.get("营业总收入同比增长率"))
            result["profit_yoy"] = _parse_pct(row.get("净利润同比增长率"))
            result["net_profit_deducted_yoy"] = _parse_pct(row.get("扣非净利润同比增长率"))
            result["debt_ratio"] = _parse_pct(row.get("资产负债率"))
            result["eps"] = _parse_pct(row.get("基本每股收益"))
            result["net_profit"] = str(row.get("净利润", ""))
            result["rev"] = str(row.get("营业总收入", ""))
        except Exception:
            pass

    # ── 3. PE 备选方案 ──
    if result["pe"] is None:
        # 3a. price/eps 计算（已有当前价和历史 eps）
        if result["eps"] is not None and result["eps"] > 0:
            try:
                price = _get_current_price(code)
                if price and price > 0:
                    calculated_pe = round(price / result["eps"], 2)
                    if 0 < calculated_pe < 10000:
                        result["pe"] = calculated_pe
            except Exception:
                pass

    if result["pe"] is None or result["pb"] is None:
        # 3b. 从东方财富实时行情取 PE/PB（轻量级调用，缓存全市场避免重复请求）
        try:
            _pe_pb_from_spot(code, result)
        except Exception:
            pass
...
# 东方财富实时行情缓存（stock_zh_a_spot_em 拉取全市场股票，有 PE/PB 列）
_SPOT_CACHE: Optional[pd.DataFrame] = None

def _pe_pb_from_spot(code: str, result: dict) -> None:
    """从全市场实时行情缓存中补全 PE/PB（实时行情始终优先）"""
    global _SPOT_CACHE
    if _SPOT_CACHE is None:
        _SPOT_CACHE = ak.stock_zh_a_spot_em()
    if _SPOT_CACHE is not None and len(_SPOT_CACHE) > 0:
        spot = _SPOT_CACHE[_SPOT_CACHE["代码"] == code]
        if len(spot) > 0:
            row = spot.iloc[0]
            pe_v = row.get("市盈率-动态")
            if pe_v is not None and pd.notna(pe_v):
                result["pe"] = float(pe_v)  # 始终覆盖，实时行情最可靠
            pb_v = row.get("市净率")
            if pb_v is not None and pd.notna(pb_v):
                result["pb"] = float(pb_v)
...
def run_layer4(codes: List[str] = None,
    ...
    for p in results:
        ...
        try:
            ...
            p.cash_to_profit_ratio = deep.get("cash_to_profit")  # None 表示无数据，UI 显示 N/A
            p.rd_ratio = deep.get("rd_ratio")  # None 表示无数据
        except Exception:
            pass
...
class StockDecision:
    ...
    debt_ratio: float = 0.0
    # 产业链周期
    cycle_phase: str = ""           # EARLY / MID_RALLY / LATE / DECLINING
    cycle_maturity: int = 50        # 0-100 成熟度
    cycle_remaining_months: float = 0  # 剩余上涨月数
    cycle_discount: float = 1.0     # 周期折扣系数（0.3~1.0）
...
def get_support_level(code: str) -> float:
    """获取最近支撑位（近20日最低点）"""
    from strategy_config import DEFAULT_STOP_LOSS
    try:
        df = _get_kline_cached(code)
...
def calculate_position(layers_passed: int, market_env: str,
                       stop_loss_pct: float, timing_signal: str,
                       cycle_maturity: int = 50) -> dict:
    from strategy_config import (
        ENV_SINGLE_CAP, ENV_TOTAL_CAP, LAYER_RATIO, TIMING_BONUS,
        STOP_LOSS_MODIFIER, FIRST_BATCH_RATIO, CYCLE_DISCOUNT_RANGES,
    )
    single_cap = ENV_SINGLE_CAP.get(market_env, 6.0)
    layer_ratio  = LAYER_RATIO.get(min(layers_passed, 5), 0)
    timing_ratio = TIMING_BONUS.get(timing_signal, 0.6)

    # ── 周期成熟度折扣 ──
    cycle_ratio = 1.0
    for maturity_limit, discount in CYCLE_DISCOUNT_RANGES:
        if cycle_maturity <= maturity_limit:
            cycle_ratio = discount
            break

    base_pct   = single_cap * layer_ratio * timing_ratio * cycle_ratio
    actual_pct = base_pct * (STOP_LOSS_MODIFIER / stop_loss_pct) if stop_loss_pct > 0 else base_pct
    actual_pct = min(actual_pct, single_cap)
    first_batch = actual_pct * FIRST_BATCH_RATIO
    return {
        "single_cap": round(single_cap, 1),
        "total_cap": ENV_TOTAL_CAP.get(market_env, 30.0),
...
def make_decision(profile: CompanyProfile,
                  ...
                  lifecycle_data: dict = None) -> StockDecision:
    """综合六层做出最终决策"""
    # ── 技术指标分析 ──
    ...
    from strategy_config import L1_PASS_THRESHOLD, L2_PASS_THRESHOLD
    d = StockDecision(
        code=profile.code,
        name=profile.name,
        chain=profile.chain,
        l0_pass=l0_pass,
        l1_pass=l1_score >= L1_PASS_THRESHOLD,
        l2_pass=l2_score >= L2_PASS_THRESHOLD,
        l3_pass=l3_active,
    ...
    cycle_maturity = 50
    cycle_info = None
    if lifecycle_data and profile.chain in lifecycle_data:
        cycle_info = lifecycle_data[profile.chain]
        d.cycle_phase = cycle_info.get("phase", "")
        d.cycle_maturity = cycle_info.get("maturity", 50)
        d.cycle_remaining_months = cycle_info.get("remaining_months", 0)
        d.cycle_discount = cycle_info.get("cycle_discount", 1.0)
    ...
    from strategy_config import L4_PASS_THRESHOLD
    layers = sum([l0_pass, d.l1_pass, d.l2_pass, d.l3_pass,
                  profile.total_score >= L4_PASS_THRESHOLD])
    ...
    if price and price > 0:
        ...
        if lots < 1:
            from strategy_config import MIN_LOTS
            lots = MIN_LOTS  # 至少1手
    ...
    if layers >= 4 and has_timing:
        ...
        if cycle_info:
            if d.cycle_maturity >= 70:
                cycle_note = (f"（产业链周期成熟度{d.cycle_maturity}/100，"
                              f"仓位已打{d.cycle_discount:.0%}折，剩余约{d.cycle_remaining_months:.0f}个月）")
            elif d.cycle_maturity <= 40:
                cycle_note = (f"（产业链处于{cycle_info.get('phase_name','')}，"
                              f"周期充裕剩余约{d.cycle_remaining_months:.0f}个月）")
        d.reason  = f"六层通过{layers}层，时机信号：{d.timing_signal}{cycle_note}"
    ...
    # 周期尾声警告
    if cycle_info and d.verdict == "GO" and d.cycle_maturity >= 75:
...
def run_layer5(profiles: List[CompanyProfile], l0_pass: bool,
               ...
               market_env: str = "SIDEWAYS",
               capital_total: float = 1_000_000,
               lifecycle_data: dict = None) -> List[StockDecision]:
    decisions = []
    for p in profiles:
        if p.verdict == "FAKE":
            continue
        l3_active = any(p.chain in c for c in l3_active_chains)
        d = make_decision(p, l0_pass, l1_score, l2_score, l3_active,
                          market_env, capital_total,
