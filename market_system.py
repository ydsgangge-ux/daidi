...
def calc_position(layers: int, env: str, stop_pct: float, timing: str) -> dict:
    ...
    base = single_cap * ratios.get(min(layers,5),0) * t_bonus.get(timing,0.6)
...
def run_layer5(profiles, l0_pass, l1_score, l2_score,
               active_chains, market_env="SIDEWAYS") -> List[StockDecision]:
    decisions = []
    for p in profiles:
        ...
        if layers>=4 and d.timing_signal in ("BREAKOUT","PULLBACK","ACCUMULATION"):
            d.verdict = "GO"
            d.reason  = f"六层通过{layers}层，{timing_info['note']}"
