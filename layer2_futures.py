...
def cross_check_l1_l2(l1_score: int, l2_signals: list) -> list:
    """Layer 1 与 Layer 2 背离检测（阈值从 config 读取）"""
    from strategy_config import L1_L2_DIVERGENCE_THRESHOLD
    divergence = []
    ...
    if diff > L1_L2_DIVERGENCE_THRESHOLD:
        divergence.append(
            f"⚠ 背离：L1实业偏强({l1_score})但L2金融偏弱({l2_score})，"
            "资金可能掌握更多信息，降仓50%等待收敛"
        )
    elif diff < -L1_L2_DIVERGENCE_THRESHOLD:
        divergence.append(
...
def run_layer2(l1_score: int = 50) -> Layer2Result:
    ...
    from strategy_config import L2_BASE_SCORE, L2_SIGNAL_WEIGHT
    result.score = int(L2_BASE_SCORE + (bullish - bearish) / max(len(all_sig), 1) * L2_SIGNAL_WEIGHT)
