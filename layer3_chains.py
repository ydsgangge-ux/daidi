...
class Layer3Result:
    ...
    timestamp: str = ""
    # 产业链周期分析
    lifecycle: dict = field(default_factory=dict)  # {chain_name: CyclePhase dict}
...
def run_layer3(l1_result, l2_result, intl_signals=None) -> Layer3Result:
    result = llm_match_chains(l1_result, l2_result, intl_signals)

    # ── 产业链周期分析 ──
    try:
        from chain_lifecycle import analyze_all_chains, cycle_to_dict
        active_names = [c.name for c in result.active_chains]
        strengths = {c.name: c.signal_strength for c in result.active_chains}
        lifecycle = analyze_all_chains(active_names, strengths)
        result.lifecycle = {k: cycle_to_dict(v) for k, v in lifecycle.items()}
        for chain in result.active_chains:
            if chain.name in result.lifecycle:
                lc = result.lifecycle[chain.name]
                chain.reasoning += f" | 周期：{lc['phase_name']}，剩余约{lc['remaining_months']}个月，成熟度{lc['maturity']}/100"
    except Exception as e:
        print(f"  [WARN] 周期分析失败: {e}")
